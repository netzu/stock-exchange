...
public class test {
	...
	public static void main(String[] args) throws ClassNotFoundException, SQLException, ParseException, FileNotFoundException, UnsupportedEncodingException {
		...
		ArrayList<DateTime> buySignlaList = signal.buySignals(collection);
		...
		for(int i=0; i<buySignlaList.size();i++){
			ArrayList<Boolean> daysWithPositiveResults = testOfBuySignalEffectiveness.daysWithPositiveResults(30, buySignlaList.get(i), stockCollectionForTicker);
			ArrayList<Double> percentageGainPerDay = testOfBuySignalEffectiveness.percentageGainPerDay(30, buySignlaList.get(i), stockCollectionForTicker);
